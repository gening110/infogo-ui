...
:::demo
```html
<el-tree :data="data" node-key="label" :current-node-key="'一级 1'" :props="defaultProps" @node-click="handleNodeClick"></el-tree>
带连接线
<el-tree :data="data" ligature :props="defaultProps" @node-click="handleNodeClick"></el-tree>
<script>
  export default {
    data() {
      return {
...
设置`height`和`node-key`开启虚拟滚动,常用于大量数据。

:::demo
```html
<el-tree :data="data" height="150px"  :node-key="'label'" :props="defaultProps" @node-click="handleNodeClick"></el-tree>
带连接线
<el-tree :data="data" height="250px" ligature :node-key="'label'" :props="defaultProps" @node-click="handleNodeClick"></el-tree>
<script>
  export default {
    data() {
      return {
  ...
  export default {
    ...
    methods: {
      filterNode(value, data, node) {
        ...
        // console.log(parentNode, labels, level)
...
| height              | 设置树的高度，用于开启虚拟滚动 （有值则开启虚拟滚动）              | String （具体像素值或者百分比）                     | —    | —     |
| itemSize              | 开启虚拟滚动后每个节点的高度（便于计算）              | String / number                      | —    | 36     |
| ligature              | 是否显示连接线              | boolean （具体像素值或者百分比）                     | —    | false|
